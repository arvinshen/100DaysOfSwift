...
//  ExplodingMonkeys
//
//  Created by Arvin Shen on 1/19/22.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func launch(angle: Int, velocity: Int) {
        // 1. Figure out how hard to throw the banana. Function accept a velocity parameter, but will divide by 10.
        let speed = Double(velocity) / 10.0
        
        // 2. Convert the input angle to radians.
        let radians = deg2rad(degrees: angle)
        
        // 3. If somehow there's a banana already, remove it then create a new one using circle physics.
        if banana != nil {
