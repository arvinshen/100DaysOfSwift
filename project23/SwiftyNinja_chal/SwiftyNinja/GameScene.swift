...
//  SwiftyNinja
//
//  Created by Arvin Shen on 1/10/22.
//
...
class GameScene: SKScene {
    ...
    var popupTime = 0.9  // amount of time to wait between last enemy being destroyed and new one being created.
    var sequence = [SequenceType]()
    var sequencePosition = 0
    var chainDelay = 3.0  // how long to wait before creating a new enemy when the sequence type is .chain or .fastChain. Enemy chains don't wait until the previous enemy is offscreen before creating a new one.
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        // 4. Remove any actions that are currently attached to the slice shapes.
        // Important if they are in the middle of a fadeOut(withDuration:) action.
        activeSliceBG.removeAllActions()
        activeSliceFG.removeAllActions()
        
        // 5. Set both slice shapes to have an alpha value of 1 so they're fully visible.
        activeSliceBG.alpha = 1
        activeSliceFG.alpha = 1
    }
    
    func redrawActiveSlice() {
        // 1. If we have fewer than 2 points in our array, we don't have enough data to draw a line so it needs to clear the shapes and exit method.
        ...
        // 3. Start its line at the position of the first swipe point, then go through each of the others drawing lines to each point.
        ...
        // 4. Update the slice shape paths so they get drawn using their designs = i.e., line width and color.
    ...
    func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        if enemyType == 0 {
            // 1. Create a new SKSpriteNode that will hold the fuse and bomb image as children, setting its zPosition to be 1.
            enemy = SKSpriteNode()
            enemy.zPosition = 1
            ...
            // 4. Create an new bomb fuse sound effect, then play it.
